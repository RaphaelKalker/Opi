...
int showdatawindow::dumpData(int data, QString fileName)
{
	...
	//if succesfully send, flush buffer
	...
	/* 
	set QIODevice::Text flag to have newline characters written using the local encoding - i.e. makes \n work like a boss.
	set QIODevice::ReadWrite for read and write access, otherwise it will just erase what was previously there
	set QIODevice::Truncate to remove what was in the file initially
	set QIODevice::Append 
	
	*/
	outputFile.open(QIODevice::ReadWrite | QIODevice::Text | QIODevice::Append);

	//check if it opened
	if (!outputFile.isOpen())
	{
		qDebug() << "Error - couldn't open output file called: " << fileName;
		return 1;
	}

	//Point a QTextstreamobject at the file
...
void showdatawindow::dumpArrayToSeperateFiles(int data [])
{
	//create a string for file name
	qint64 epoc = QDateTime::currentMSecsSinceEpoch();
	//std::string fileName = "out" + std::to_string(epoc);
	QString fileName = "out_" + QString::number(epoc) + ".txt";
	
	//create file with this string name
	QFile outputFile(fileName);

	qDebug() << fileName;

	//open file, ensure it doesn't fail
	outputFile.open(QIODevice::ReadWrite | QIODevice::Text | QIODevice::Append);
...
int showdatawindow::routinedrawgroup(void)
{
    ...
    if(newdata&&(!ui->CheckPauseShow->isChecked()))  //show the data when get new package
    {
        ...
        for( i= 0;i< NUMPOINTSAMPLE; i++)
        {
			...
			//qDebug() << adc_new_data[i];
			//dumpData(adc_new_data[i], "adc_output");
			
			dumpArrayToSeperateFiles(adc_new_data);
        }
