...
startwindow::startwindow(qint32 *comPortUser, QWidget *parent) :
    ui(new Ui::startwindow)
{
    ...
    pdnshowcountp = 0;
    for(i =0;i<OPIWANT_MAX+1;i++)
        sdw[i] = new showdatawindow(&pdnshowcountp,this);
...
/checks if recording is activated
*/
...
void startwindow::hide_check_pdn()
{
    ui->CBP1->hide();
    ui->CBP2->hide();
    ui->CBP3->hide();
...
void startwindow::show_check_pdn()
{
    int i;
...
void startwindow::on_CBP1_clicked()
{
    PDNBOX[1]= !PDNBOX[1];
    if(PDNBOX[1])
    {
...
void startwindow::on_CBP2_clicked()
{
    PDNBOX[2]= !PDNBOX[2];
    if(PDNBOX[2])
    {
...
void startwindow::on_CBP3_clicked()
{
    PDNBOX[3]= !PDNBOX[3];
    if(PDNBOX[3])
    {
...
void startwindow::on_CBP4_clicked()
{
    PDNBOX[4]= !PDNBOX[4];
    if(PDNBOX[4])
    {
...
/* 
checks com port
writes a portion of the payload to an opi file - perhaps this is header info???
*/
void startwindow::on_pushButton_2_clicked()
{
    ...
    else
    {
        *comPortUserp = PDCCOMPORTUSER;
        ...
        if(firsttimer)
        {
            timeid=QObject::startTimer(startwindow_fresh_time);
            firsttimer=false;
        }
        fresh();
    }
...
int startwindow::opiucd_getPDN(void)
{
    int totalcount=0, i;
...
void startwindow::closeEvent(QCloseEvent *)
{
    ...
    else if(portOn&&get_opipkt_total)
    {
        firsttimer=true;
        if(portOn)
            opi_closeucd_com(&com);
    ...
    {
        qApp->processEvents();
        if(sdw[i]->already_open||sdw[i]->isVisible())
            sdw[i]->close();
    }
    for(i=0; i<OPIWANT_MAX+1; i++)
    {
...
void startwindow::borrowCOM(void)
{
    if(portOn)
        opi_closeucd_com(&com);
    if(!firstfile)
...
void startwindow::returnCOM(void)
{
...
/***
  ...
  *			-1	// error
  */
int startwindow::opipkt_put_file(OPIPKT_t* pktptr, FILE* fileptr)
{
    quint32 i;
    quint16 calcChksm=0;
    quint8 tempuint8;
