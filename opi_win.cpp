...
int opiucd_settsrftxpwr(HANDLE *comportptr, int rftxpwr)
{
    ...
    //opipkt.payload[1] = rftxpwr & 0x64; //100 in hex
    opipkt.length = 2;
    if (opipkt_put_com(&opipkt, comportptr) != 0) return -1;
    if (opipkt_get_com(&opipkt, comportptr) != 0) return -1;
    if (opipkt.dataCode == 0x40)        return 0;
...
/***
  *	Request 
  wireless truesense data from unicon (received from truesense).
...
{
	qDebug() << "opiucd_get5mmtsdata - " << "pktnumber:  " << pktNumber;
...
int opiucd_onmode(HANDLE *comportptr)
{
	...
	qDebug() << "Set controller to on-state where it will turn on plugged in truesense, invoked opiucd_onmode()";
